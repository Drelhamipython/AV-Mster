const App = (() => {
  ...
  function openForm(t,r={}){
    ...
    const forms={
      ...
      site_reports:`${selectHtml('project_id','Project',projectOptions(r.project_id))}${field('report_date','Date',r.report_date||date(),'date')}${area('work_completed','Work Completed',r.work_completed)}${area('issues','Issues / Delays',r.issues||r.delays)}<div class="field"><label>Report Photos</label><input id="files" type="file" accept="image/*" capture="environment" multiple><small class="muted">Take photos with the device camera or upload images from the device.</small><div id="previews" class="photos" style="margin-top:12px"></div></div>`,
  ...
  const allowedColumns={
    ...
    site_reports:['project_id','report_date','work_completed','issues','submitted_by','submitted_by_name','submitted_by_email'],
