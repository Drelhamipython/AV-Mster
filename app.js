const App = (() => {
  ...
  function recentSiteReports(){return `<div class="card"><div class="between"><h2>Recent Site Reports</h2><button class="btn ghost" data-go="site_reports">View all</button></div>${cache.site_reports.slice(0,5).map(r=>siteReportRow(r)).join('')||'<p class="muted">No site reports yet.</p>'}</div>`}
