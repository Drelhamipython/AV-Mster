const App = (() => {
  ...
  let sb=null, user=null, profile=null, active='dashboard', selectedPhotoFiles=[], cache={projects:[],plans:[],tasks:[],photos:[],daily_logs:[],rfis:[],subcontractors:[]};
  ...
  function renderLogin(){
    ...
    $('#resetCfg').onclick=()=>{localStorage.removeItem(LS_CFG);location.reload()};
  }
  function renderShell(){
    document.body.innerHTML=`<div class="app"><aside class="sidebar" id="side"><div class="logo"><img class="brandLogo" src="assets/amin-logo-full.png" alt="Amin Ventures full logo"></div><nav class="nav">${navBtn('dashboard','⌂','Dashboard')}${navBtn('projects','▣','Projects')}${navBtn('plans','▤','Plans')}${navBtn('tasks','☑','Tasks / Punch List')}${navBtn('photos','▧','Photos')}${navBtn('daily_logs','◷','Daily Logs')}${navBtn('rfis','?','RFIs')}${navBtn('subcontractors','♟','Subcontractors')}${navBtn('reports','▥','Reports')}${navBtn('users','◉','Users / Roles')}${navBtn('cloud','⚙','Cloud Setup')}</nav><div class="userbox"><b>${esc(profile.full_name||profile.email)}</b><br>${esc(profile.role)}<br><span class="muted">${esc(profile.email)}</span><br><br><div class="muted" style="margin-top:10px">CLOUD v5.7 • Supabase connected</div><button class="btn ghost" id="signout">Sign out</button></div></aside><main class="main"><header class="topbar"><button class="btn mobileMenu" id="menu">☰</button><input class="search" id="search" placeholder="Search projects, tasks, plans..."><div class="top-actions"><button class="btn" id="sync">Sync</button><button class="btn gold" id="quickTask">+ New Task</button></div></header><section class="content" id="view"></section></main></div>`;
    $('#signout').onclick=async()=>{await sb.auth.signOut();location.reload()}; $('#sync').onclick=async()=>{await loadAll(); renderView();}; $('#quickTask').onclick=()=>openForm('tasks'); $('#menu').onclick=()=>$('#side').classList.toggle('open'); document.querySelectorAll('[data-nav]').forEach(b=>b.onclick=()=>{active=b.dataset.nav;renderShell()}); renderView();
  }
  ...
  function dashboard(){
    ...
    return `<div class="between"><div><h1>Command Center</h1><p class="muted">Real cloud database, cloud photo storage, multi-user access, and role permissions.</p></div><button class="btn gold no-print" data-add="tasks">+ New Task</button></div><div class="grid stats"><div class="card stat"><div class="label">Projects</div><div class="value">${cache.projects.length}</div><div class="sub">Cloud synced</div></div><div class="card stat"><div class="label">Open Tasks</div><div class="value">${open}</div><div class="sub">Assigned work</div></div><div class="card stat"><div class="label">Critical</div><div class="value">${critical}</div><div class="sub">Needs attention</div></div><div class="card stat"><div class="label">Photos</div><div class="value">${cache.photos.length}</div><div class="sub">Supabase Storage</div></div></div><div class="grid" style="grid-template-columns:1.1fr 1fr;">${recentProjects()}${recentTasks()}</div><br><div class="grid" style="grid-template-columns:1fr 1fr;">${recentPhotos()}${recentLogs()}</div>`;
  }
  ...
  function listPage(t){
    ...
    return `<div class="between"><div><h1>${titles[t]}</h1><p class="muted">Create, edit, delete, and sync records to Supabase.</p></div>${canEdit()?`<button class="btn gold no-print" data-add="${t}">+ Add</button>`:''}</div><br><div class="card">${tableFor(t)}</div>`;
  }
  function tableFor(t){
    if(t==='photos') return `<div class="photos">${cache.photos.map(p=>photoCard(p,true)).join('')||'<p class="muted">No photos.</p>'}</div>`;
    const rows=cache[t]; const cols={projects:['name','address','status'],plans:['sheet_no','title','revision','project_id'],tasks:['title','project_id','location','priority','status','due_date'],daily_logs:['log_date','project_id','weather','manpower','work_completed'],rfis:['title','project_id','status','due_date'],subcontractors:['company','trade','contact_name','phone','email']}[t];
  ...
  function bindActions(){
    ...
    const lu=$('#loadUsers'); if(lu) lu.onclick=loadUsers; const sc=$('#saveCloud'); if(sc) sc.onclick=()=>{setCfg({url:$('#cloud_url').value.trim(),anonKey:$('#cloud_key').value.trim()}); location.reload()}; const rc=$('#resetCloud'); if(rc) rc.onclick=()=>{localStorage.removeItem(LS_CFG); location.reload()};
  }
